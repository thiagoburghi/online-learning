...
function HCO_sde(du,u,p,t)
    ...
    for n = 1:2
        du[0+n]=1/C[n]*(- gNa[n]*m[n]^3*h[n]*(V[n]-VNa) - gH[n]*mH[n]*(V[n]-VH) - gT[n]*mt[n]^2*ht[n]*(V[n]-VCa) - gA[n]*mA[n]^4*hA[n]*(V[n]-VK) + 
                        ...
                        - gsyn[n]*msyn[n]*(V[n]-Esyn[n]) + Iapp[n](t) + noise[n] )
        ...
        du[24+n]=-noise[n]/taunoise[n]
    end
end

function HCO_noise(du,u,p,t)
    du[1:24] = zeros(1,24)
    du[25:26].=p[16]
end

function HCO_observer_sde(du,u,p,t)
    Iapp=p[1] 
    gT=p[2]
    gKD=p[3]
    gH=p[4]
...
function HCO_observer_noise(du,u,p,t)
    du[1:246] = zeros(1,246)
    du[247:248] .= p[16]
end